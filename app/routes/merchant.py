...
@merchant_bp.route('/api/list', methods=['GET'])
@login_required
@check_api_permission('merchant_view')
def api_list():
    """商户列表API"""
    try:
        ...
        merchants, total_count, total_pages = MerchantService.get_merchants(
        ...
        for m in merchants:
            items.append({
                ...
                'merchant_name': m.merchant_name or '',
                'legal_person': m.legal_person or '',
                'contact_person': m.contact_person or '',
                'phone': m.phone or '',
                'merchant_type': m.merchant_type or '',
                'business_type': m.business_type or '',
...
@login_required
@check_api_permission('merchant_edit')
