    ...
    with DBConnection() as conn:
        ...
        cursor.execute("""
            SELECT 
                ...
                ISNULL(SUM(CASE WHEN Status = N'已收' THEN TotalAmount ELSE 0 END), 0),
        ...
        cursor.execute("""
            SELECT 
                ISNULL(SUM(CASE WHEN Status = N'未付' THEN RemainingAmount ELSE 0 END), 0),
                ISNULL(SUM(CASE WHEN Status = N'已付' THEN TotalAmount ELSE 0 END), 0)
            FROM Payable
        """)
        ...
        # 4. 在租商户数 & 空置率
        cursor.execute("""
            SELECT COUNT(*) FROM Merchant WHERE Status = N'正常'
        """)
        ...
        cursor.execute("""
            SELECT TOP 5
                cf.Direction, cf.Amount, cf.ExpenseItemName,
        ...
        # 7. 逾期应收
        cursor.execute("""
            SELECT TOP 5 r.ReceivableID, r.TotalAmount, r.RemainingAmount, 
                         r.DueDate, r.Description, c.CustomerName
            FROM Receivable r
            LEFT JOIN Customer c ON r.CustomerID = c.CustomerID
            WHERE r.Status IN (N'未收', N'部分收')
        ...
        # 8. 即将到期合同（30天内）
        cursor.execute("""
            SELECT TOP 5 ContractID, ContractNumber, MerchantName, EndDate, Status
            FROM Contract
            WHERE Status = N'生效'
