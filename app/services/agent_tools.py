...
def _register_all_tools(reg):
    ...
    _register_overview_tools(reg, merchant_svc, contract_svc, finance_svc, receivable_svc, utility_svc)
...
def _register_overview_tools(reg, merchant_svc, contract_svc, finance_svc, receivable_svc, utility_svc):
    ...
    }, _make_merchant_overview_executor(merchant_svc, contract_svc, finance_svc, receivable_svc, utility_svc))


def _make_merchant_overview_executor(merchant_svc, contract_svc, finance_svc, receivable_svc, utility_svc):
    def executor(merchant_id=None, merchant_name=None, _merchant_id=None, _source='admin'):
        if _source == 'wx' and _merchant_id:
            merchant_id = _merchant_id
        ...
        # 基本信息
        info = merchant_svc.get_merchant_by_id(merchant_id)
        if not info:
            return {'error': u'未找到该商户'}
...
def _register_utility_tools(reg, utility_svc):
    ...
    }, _make_electricity_stats_executor())
...
def _make_electricity_stats_executor():
...
def _make_water_stats_executor():
    def executor(group_by_month=False, _merchant_id=None, _source='admin'):
        from utils.database import DBConnection
        with DBConnection() as conn:
            cursor = conn.cursor()
