...
8. Routes → Services → DBConnection 三层分离（Routes层禁止SQL）
...
10. 一个模块，一组路由
    ...
    财务 /finance
11. 表单提交方式：
    必须使用 Ajax，禁止传统 form submit（页面刷新），CSRF 令牌 （Ajax POST必须携带），列表删除后必须异步刷新
12. 文件统一管理
    所有文件必须走 /uploads/，数据库存路径，不存文件
13. SQL规范：必须使用参数化查询，禁止拼接SQL
    正确示例：
    cursor.execute("SELECT \* FROM merchant WHERE id = ?", (merchant\_id,))
14. 所有功能的实现（含页面+后端）均按照Spec进行，有模糊的地方，需要与我沟通确认
15. 当我让你实现功能时，必须按照spec全局约束（强制执行）进行，先查看spec中是否有相关描述，并分析相关描述，没有则与我沟通确认
