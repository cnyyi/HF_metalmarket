...
| 决策项 | 选择 | 理由 |
|--------|------|------|
| 操作粒度 | CRUD 四级（view/create/edit/delete） | 覆盖绝大多数业务场景，不过度设计 |
...
`{module}_{operation}` 两段式，operation 固定为 `view` / `create` / `edit` / `delete`。
...
| prepayment_view | 预收预付查看 | 财务管理 | view | 1101 |
| prepayment_create | 预收预付新增 | 财务管理 | create | 1102 |
| prepayment_edit | 预收预付编辑 | 财务管理 | edit | 1103 |
| prepayment_delete | 预收预付删除 | 财务管理 | delete | 1104 |
| deposit_view | 押金查看 | 财务管理 | view | 1201 |
| deposit_create | 押金新增 | 财务管理 | create | 1202 |
| deposit_edit | 押金编辑 | 财务管理 | edit | 1203 |
| deposit_delete | 押金删除 | 财务管理 | delete | 1204 |
| customer_view | 往来客户查看 | 财务管理 | view | 1301 |
| customer_create | 往来客户新增 | 财务管理 | create | 1302 |
| customer_edit | 往来客户编辑 | 财务管理 | edit | 1303 |
| customer_delete | 往来客户删除 | 财务管理 | delete | 1304 |
| salary_view | 工资查看 | 财务管理 | view | 1401 |
| salary_create | 工资新增 | 财务管理 | create | 1402 |
| salary_edit | 工资编辑 | 财务管理 | edit | 1403 |
| salary_delete | 工资删除 | 财务管理 | delete | 1404 |
...
- `OperationCode`：操作类型，值为 `view` / `create` / `edit` / `delete`，旧权限标记为 `manage`
...
```sql
UPDATE Permission SET OperationCode = 'manage' WHERE PermissionCode LIKE '%_manage' OR PermissionCode IN ('direct_entry', 'utility_reading');
```

### 2.3 ModuleName 规范化

```sql
UPDATE Permission SET ModuleName = N'市场管理' WHERE PermissionCode IN ('plot_view','plot_create','plot_edit','plot_delete','merchant_view','merchant_create','merchant_edit','merchant_delete','contract_view','contract_create','contract_edit','contract_delete','utility_view','utility_create','utility_edit','utility_delete','scale_view','scale_create','scale_edit','scale_delete','expense_view','expense_create','expense_edit','expense_delete','garbage_view','garbage_create','garbage_edit','garbage_delete','dorm_view','dorm_create','dorm_edit','dorm_delete');
UPDATE Permission SET ModuleName = N'财务管理' WHERE PermissionCode IN ('finance_view','finance_create','finance_edit','finance_delete','prepayment_view','prepayment_create','prepayment_edit','prepayment_delete','deposit_view','deposit_create','deposit_edit','deposit_delete','customer_view','customer_create','customer_edit','customer_delete','salary_view','salary_create','salary_edit','salary_delete');
UPDATE Permission SET ModuleName = N'系统管理' WHERE PermissionCode IN ('user_view','user_create','user_edit','user_delete','role_view','role_create','role_edit','role_delete','permission_view','permission_create','permission_edit','permission_delete','dict_view','dict_create','dict_edit','dict_delete');
```
...
原来拥有 `merchant_manage` 的角色，自动获得对应的 CRUD 四级权限：
...
INNER JOIN Permission p ON p.ModuleName = old_p.ModuleName
    AND p.OperationCode IN ('view','create','edit','delete')
    AND SUBSTRING(p.PermissionCode, 1, LEN(p.PermissionCode) - CHARINDEX('_', REVERSE(p.PermissionCode)))
        = SUBSTRING(old_p.PermissionCode, 1, LEN(old_p.PermissionCode) - CHARINDEX('_', REVERSE(old_p.PermissionCode)))
WHERE old_p.OperationCode = 'manage'
    AND NOT EXISTS (
...
按 ModuleName 分组，每组内按模块排列，每个模块一行 4 个 checkbox：
...
- 勾选"新增/编辑/删除"时，自动勾选"查看"（没有查看权限就无法进入页面）
...
侧边栏菜单项的可见性由 `xxx_view` 控制（与旧权限行为一致，因为旧权限映射到 view）：
...
class Permission:
    def __init__(self, permission_id=None, permission_name=None,
        ...
        self.operation_code = operation_code   # 新增：view/create/edit/delete/manage
...
1. **数据库迁移脚本**：添加 OperationCode、SortOrder 字段，插入新权限，迁移角色赋权，标记旧权限
2. **模型层**：Permission 模型新增字段，User 模型增加 LEGACY_PERMISSION_MAP 和兼容逻辑
3. **路由层**：逐模块替换 check_permission 参数（旧代码仍可工作，可分批改造）
4. **模板层**：侧边栏改用 `xxx_view`，页面内按钮改用 `xxx_create/edit/delete`
5. **权限管理页面**：新增角色编辑页的树形权限分配界面
6. **验证**：确保旧权限兼容、新权限生效、前端展示正确
